...
import com.galgoda.member.model.vo.*;
import com.galgoda.supervisor.model.service.SupervisorService;

/**
 * Servlet implementation class SupervisorUserListController
 */
@WebServlet("/userList.su")
public class SupervisorUserListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SupervisorUserListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new SupervisorService().selectUserListCount();
		...
		List<Customer> list = new SupervisorService().selectUserList(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/views/supervisor/supervisorUserList.jsp").forward(request, response);
	}
